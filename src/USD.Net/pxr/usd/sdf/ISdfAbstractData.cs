...
public interface ISdfAbstractData
{
    ...
    /// </summary>
    void CreateSpec(ISdfPath path, SdfSpecType specType);
    ...
    /// </summary>
    bool Has(ISdfPath path, TfToken fieldName, SdfAbstractDataValue? value);

    /// <summary>
    /// Return true if this data has the field identified by path and fieldName,
    /// false otherwise. If value is provided, return the value if the field exists.
    /// </summary>
    bool Has(ISdfPath path, TfToken fieldName, VtValue? value = null);
    ...
    /// </summary>
    bool HasSpecAndField(ISdfPath path, TfToken fieldName, SdfAbstractDataValue? value, out SdfSpecType specType);
    ...
    /// return the spec type if the spec exists. Default implementation calls HasSpec() and Has().
    /// </summary>
    bool HasSpecAndField(ISdfPath path, TfToken fieldName, VtValue? value, out SdfSpecType specType);

    /// <summary>
    /// Return the value of the field identified by path and fieldName.
    /// Returns an empty VtValue if the field doesn't exist.
    /// </summary>
    VtValue Get(ISdfPath path, TfToken fieldName);
    ...
    /// </summary>
    Type GetTypeid(ISdfPath path, TfToken fieldName) => Get(path, fieldName).GetType();
    ...
    /// </summary>
    T GetAs<T>(ISdfPath path, TfToken fieldName, T defaultValue = default!);

    /// <summary>
    /// Set the field identified by path and fieldName to value,
    /// or remove the field if value is empty.
    /// </summary>
    void Set(ISdfPath path, TfToken fieldName, VtValue value);

    /// <summary>
    /// Set the field identified by path and fieldName to value,
    /// or remove the field if value is empty.
    /// </summary>
    void Set(ISdfPath path, TfToken fieldName, SdfAbstractDataConstValue value);

    /// <summary>
    /// Erase the field identified by path and fieldName, if it exists.
    /// </summary>
    void Erase(ISdfPath path, TfToken fieldName);

    /// <summary>
    /// Return a vector of the fieldNames for the spec at path.
    /// Returns an empty vector if the spec doesn't exist.
    /// </summary>
    List<TfToken> List(ISdfPath path);
    ...
    /// </summary>
    bool HasDictKey(ISdfPath path, TfToken fieldName, TfToken keyPath, SdfAbstractDataValue? value);
    ...
    /// the value associated with the key if it exists.
    /// </summary>
    bool HasDictKey(ISdfPath path, TfToken fieldName, TfToken keyPath, VtValue? value = null);
    ...
    /// </summary>
    VtValue GetDictValueByKey(ISdfPath path, TfToken fieldName, TfToken keyPath);

    /// <summary>
    /// Set the value for the given key in the dictionary-valued field at path and fieldName.
    /// If value is empty, remove the key.
    /// </summary>
    void SetDictValueByKey(ISdfPath path, TfToken fieldName, TfToken keyPath, VtValue value);

    /// <summary>
    /// Set the value for the given key in the dictionary-valued field at path and fieldName.
    /// If value is empty, remove the key.
    /// </summary>
    void SetDictValueByKey(ISdfPath path, TfToken fieldName, TfToken keyPath, SdfAbstractDataConstValue value);

    /// <summary>
    /// Remove the value for the given key in the dictionary-valued field at path and fieldName.
    /// </summary>
    void EraseDictValueByKey(ISdfPath path, TfToken fieldName, TfToken keyPath);
    ...
    /// </summary>
    List<TfToken> ListDictKeys(ISdfPath path, TfToken fieldName, TfToken keyPath);
    ...
    /// </summary>
    HashSet<double> ListTimeSamplesForPath(ISdfPath path);
    ...
    /// </summary>
    bool GetBracketingTimeSamplesForPath(ISdfPath path, double time, out double tLower, out double tUpper);
    ...
    /// </summary>
    bool GetPreviousTimeSampleForPath(ISdfPath path, double time, out double tPrevious);
    ...
    /// </summary>
    bool QueryTimeSample(ISdfPath path, double time, VtValue? optionalValue = null);
    ...
    /// If optionalValue is provided, it will be set to the value.
    /// </summary>
    bool QueryTimeSample(ISdfPath path, double time, SdfAbstractDataValue? optionalValue);
